...
function count_total_students_placed($connect)
{
	// $query = "
	// SELECT * FROM user_details WHERE user_status='active'";

	$query = "
	SELECT distinct pid FROM offer";
...
function highest_package($connect)
{
	...
	$query = "SELECT MAX(package) from offer";
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
	foreach($result as $row)
...
function average_package($connect)
{
	...
	// 	return number_format($row['total_order_value'], 2);
	// }
	$query = "SELECT AVG(package) from offer";
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
	foreach($result as $row)
	{
		// return number_format($row['total_order_value'], 2);
		return number_format($row['AVG(package)'], 2);
	}
}
